...
const APP_BUILD = 'unified.41';   // bump with sw.js CACHE_VERSION so the Relaunch row shows the live build
...
// Relaunch to update · failproof: clears the cached app files, activates any
// waiting service worker, and reloads so the newest build always loads. Data is
// untouched (it lives in the Sheet + localStorage, never in the cache).
async function relaunchToUpdate(){
  try { toast('Updating to the latest version…'); } catch(e){}
  try {
    if ('serviceWorker' in navigator) {
      const reg = await navigator.serviceWorker.getRegistration();
      if (reg) { try { await reg.update(); } catch(e){} if (reg.waiting) { try { reg.waiting.postMessage('skipWaiting'); } catch(e){} } }
    }
  } catch(e){}
  try { if (window.caches) { const keys = await caches.keys(); await Promise.all(keys.map(k => caches.delete(k))); } } catch(e){}
  setTimeout(() => { try { location.reload(); } catch(e){ location.href = location.pathname + '?u=' + Date.now(); } }, 300);
}
...
function enterApp() {
  ...
  const rb = document.getElementById('relaunch-sub'); if (rb) rb.textContent = 'v' + APP_VERSION + ' · ' + APP_BUILD;
  renderAll();
  try { updateSyncBadge(); } catch(e){}
...
function applyRemoteData(d) {
  if (!d) return false;
  // ── Anti-wipe guard ──────────────────────────────────────────────────────
  // A transient/partial read (token hiccup, wrong sheet, network blip) can come
  // back with no transactions AND no accounts. Never let that overwrite real
  // local data — that was a silent, permanent "everything reset". If we already
  // hold data and the incoming payload is empty, keep what we have.
  const _incomingEmpty = (!d.transactions || !d.transactions.length) && (!d.accounts || !d.accounts.length);
  const _haveLocal = (S.txs && S.txs.length) || (S.accts && S.accts.length);
  if (_incomingEmpty && _haveLocal) { console.warn('applyRemoteData: ignored an empty remote read — kept local data'); return false; }
  try {
  ...
  // ── Merge, don't replace ─────────────────────────────────────────────────
  // A sync must never delete a local entry the remote read simply hasn't caught
  // up to yet (a write still in the outbox, or one that hasn't landed). We take
  // the remote list as the base and re-add any local entry that (a) is still
  // queued to write, or (b) was added in the last 15 minutes — so switching tabs
  // can never make a fresh entry vanish.
  let _pendingTxIds = new Set(), _pendingAcctIds = new Set();
  try { (outboxLoad() || []).forEach(it => { const b = it.body || {}; if (!b.id) return; if (b.action === 'addTransaction') _pendingTxIds.add(String(b.id)); if (b.action === 'addAccount') _pendingAcctIds.add(String(b.id)); }); } catch(e) {}
  const _now = Date.now(), _FRESH = 15 * 60 * 1000;
  const _remoteTx = d.transactions || [];
  const _remoteTxIds = new Set(_remoteTx.map(t => String(t.id)));
  const _keepTx = (S.txs || []).filter(t => t.id && !_remoteTxIds.has(String(t.id)) && (_pendingTxIds.has(String(t.id)) || (Number(t.ts) || 0) > _now - _FRESH));
  S.txs = sortTxs(_remoteTx.concat(_keepTx));
  const _remoteAcc = (d.accounts || []).map(a => ({ ...a, _open: Number(a.balance || 0) }));
  const _remoteAccIds = new Set(_remoteAcc.map(a => String(a.id)));
  const _keepAcc = (S.accts || []).filter(a => a.id && !_remoteAccIds.has(String(a.id))); // never drop a local account on sync
  S.accts = _remoteAcc.concat(_keepAcc);
  _reconcileBalances();
  S.members = d.members || [];
...
function _applyBetaFeature(feature, on){
  ...
  // AI Boost: turning it on must actually open the box to paste the Gemini key.
  if (feature === 'aiBoost') {
    if (on) { if (!aiKey()) { try { openAiKeySheet(); } catch(e){} return; } }
    else { try { localStorage.removeItem('prizm_quick_ai_key'); } catch(e){} try { _aiSub(); } catch(e){} }
  }
    ...
    showCustomConfirm('Restore this backup?',
      ...
      function () {
        try {
          ...
          renderAll();
          // Push the restored ledger to the Sheet through the durable outbox, so a
          // later sync can't overwrite it (dedup-by-id avoids duplicating rows the
          // Sheet already has). Without this the restore lived only on-device and
          // the next sync wiped it. (fbWriteAll was a dead no-op.)
          if (S.mode === 'sheets') { try { syncLocalToSheet(); } catch (e) { console.warn('restore→sheet:', e); } }
...
async function syncData() {
  ...
  _syncActive = true; try { updateSyncBadge(); } catch(e){}
  try { const d = await apiGet(true); if (d && !d.error && applyRemoteData(d)) renderAll(); }
  finally { _syncActive = false; try { updateSyncBadge(); } catch(e){} }
}
...
// Push the entire local ledger to the Sheet through the durable outbox. Used
// after a restore / bulk change so the Sheet reflects on-device data and a later
// sync can't overwrite it. Idempotent: addTransaction dedups by id, updateAccount
// upserts — so re-running never duplicates rows.
function syncLocalToSheet(){
  if (S.mode !== 'sheets') return;
  try {
    (S.accts || []).forEach(a => outboxEnqueue({ action:'updateAccount', id:a.id, name:a.name, type:a.type, balance:(a._open != null ? a._open : a.balance) }));
    (S.recurring || []).forEach(r => outboxEnqueue({ action:'updateRecurring', id:r.id, name:r.name, amount:r.amount, type:r.type, category:r.category, account:r.account, frequency:r.frequency, nextDate:r.nextDate, memberId:r.memberId, active:r.active !== false }));
    (S.budgets || []).forEach(b => outboxEnqueue({ action:'setBudget', id:b.id, category:b.category, limit:b.limit }));
    (S.txs || []).forEach(t => outboxEnqueue({ action:'addTransaction', id:t.id, date:t.date, description:t.description, amount:t.amount, type:t.type, category:t.category, account:t.account, notes:t.notes, memberId:t.memberId, ts:t.ts }));
    outboxFlush();
    toast('Syncing your data to Google…');
  } catch(e) { console.warn('syncLocalToSheet:', e); }
}
...
function updateSyncBadge() {
  try {
    ...
    // Always-visible pill in the home topbar
    const ss = document.getElementById('sync-status');
    if (ss) {
      if (S.mode !== 'sheets') { ss.style.display = 'none'; }
      else {
        ss.style.display = 'flex';
        if (_syncActive && n === 0) { ss.textContent = 'Syncing…'; ss.className = 'sync-status syncing'; }
        else if (stalled) { ss.textContent = 'Retrying…'; ss.className = 'sync-status warn'; }
        else if (n > 0) { ss.textContent = n + ' unsynced'; ss.className = 'sync-status warn'; }
        else if (failed > 0) { ss.textContent = failed + ' rejected'; ss.className = 'sync-status warn'; }
        else { ss.textContent = 'Synced'; ss.className = 'sync-status ok'; }
      }
    }
...
setInterval(() => { if (outboxCount()) outboxFlush(); }, 30000);
...
function startPolling() {
  ...
  if (S.mode === 'sheets') {
    ...
    const token = {};
    S.pollToken = token;
    const POLL_MIN = 20000, POLL_MAX = 90000; // calm cadence — no constant full re-render flicker
    let delay = POLL_MIN;
    const loop = async () => {
...
function _countUp(el, to, fmtFn, onFrame) {
  if (!el) return;
  // Set directly (no animation) so a background sync can never re-animate / flicker the number.
  el._cuv = to;
  el.innerHTML = fmtFn(to);
...
function renderHome() {
  ...
  if (showIncome) {
    ...
    heroTarget = netWorth;
    if (heroRowEl) { heroRowEl.style.display = ''; heroRowEl.classList.remove('single-stat'); } // two boxes → side by side
...
function renderOverviewTab(txs, tInc, tExp, catTotals) {
  ...
  if (dowEl) {
    ...
    const _maxV = Math.max.apply(null, avgs);
    const _todayIdx = new Date().getDay();
    // Colour with meaning: today = full accent, biggest-spending day = strong,
    // the rest dimmed — so the chart tells a story at a glance (not a flat bar row).
    const _bg = avgs.map((v,i) => i === _todayIdx ? _acc : (v === _maxV && _maxV > 0 ? _acc + 'B3' : _acc + '33'));
    S.dowChart = new Chart(dowEl, {
      type: 'bar',
      data: {
        labels: days,
        datasets: [{ data: avgs, backgroundColor: _bg, hoverBackgroundColor: _acc, borderRadius: 8, borderSkipped: false, maxBarThickness: 46 }]
      },
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: 'rgba(24,24,27,0.96)', titleColor:'#fff', bodyColor:'#E7CD95',
            padding: 11, cornerRadius: 12, displayColors: false, caretSize: 6,
            titleFont: { weight: '700', size: 12.5 }, bodyFont: { weight:'700', size: 14 },
            callbacks: {
              title: items => _fullDays[items[0].dataIndex],
              label: ctx => 'avg ' + fmtMoney(ctx.raw)
            }
          }
        },
        scales: {
          x: { grid:{display:false}, ticks:{ color:tickColor, font:{size:11, weight:'600'} }, border:{display:false} },
          y: { grid:{color:gridColor}, ticks:{ color:tickColor, font:{size:10}, maxTicksLimit:4, callback:v=>S.cur.sym+v }, border:{display:false} }
        }
      }
    });
  }
...
async function _gsAppend(tab, rowVals){
  var d = await gsFetch('POST', _gsBase() + '/values/' + encodeURIComponent(tab) + ':append?valueInputOption=RAW&insertDataOption=INSERT_ROWS&includeValuesInResponse=false', { values: [rowVals] });
  if (d.error) { console.error('Sheets append error [' + tab + ']:', d.error); throw new Error(d.error); }
  // Verified write: Google must confirm exactly the row landed. If it didn't, throw
  // so the outbox keeps it queued and retries — a write can never be silently lost.
  if (!d.updates || !(Number(d.updates.updatedRows) >= 1)) { var e = new Error('append not confirmed by Google'); e.transient = true; throw e; }
  return { success: true };
}
async function _gsUpdateRow(tab, row, rowVals){
  var range = "'" + tab + "'!A" + row + ':' + GS_COLS[tab] + row;
  var d = await gsFetch('PUT', _gsBase() + '/values/' + encodeURIComponent(range) + '?valueInputOption=RAW&includeValuesInResponse=false', { values: [rowVals] });
  if (d.error) { console.error('Sheets update error [' + tab + ']:', d.error); throw new Error(d.error); }
  if (!(Number(d.updatedCells) >= 1)) { var e = new Error('update not confirmed by Google'); e.transient = true; throw e; }
  return { success: true };
}
async function _gsDeleteRow(tab, row){
  ...
  if (d.error) { console.error('Sheets delete error [' + tab + ']:', d.error); throw new Error(d.error); }
  if (!d.spreadsheetId) { var e = new Error('delete not confirmed by Google'); e.transient = true; throw e; }
  return { success: true };
}
